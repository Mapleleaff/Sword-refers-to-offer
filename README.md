...
（1）float强制转换。（2）正则表达式。表示数值的字符串遵循模式 A[.[B]][e|EC] 或者 .B[e|EC]，其中 A 为数值的整数部分，B 紧跟着小数点为数值的小数部分，C 紧跟着 "e" 或者 "E" 为数值的指数部分。上述 A 和 C 都是可能以 "+" 或者 "-" 开头的 0-9 的数位串，B 也是 0-9 的数位串，但前面不能有正负号
