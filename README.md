...
（1）基于划分的方法，**时间复杂度 O(N)。**第一次划分后，划分的位置如果大于 K，就在前面子数组中继续划分，反之则在后面的子数组中继续划分。
（2）基于二叉树或者堆来实现，**时间复杂度 O(NlogK)。**首先把数组前 K 个构建一个最大堆，然后从 K+1 个数字开始遍历数组，如果遍历到的元素小于堆顶的数字，那么交换数字，重新构造堆，继续遍历。
