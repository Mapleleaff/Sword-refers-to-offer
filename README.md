...
[题42-连续子数组的最大和](题42-连续子数组的最大和.py)  
（1）对于连续子数组，用两个变量代表当前和 cursum 以及最大和 maxsum，如果当前和小于 0，那么直接把下一个数值赋给当前和；如果当前和大于 0，则累加下一个数值。同时更新 maxsum。（2）动态规划方法

[题43-1~n整数中1出现的次数](题43-1~n整数中1出现的次数.py)    
找数学规律。将数字每一位分为 0、1 以及大于等于 2 三种情况。当前位数值大于等于 2 的时候，比当前位数值等于 1 的情况，多乘了 m，考虑到 x>=2，x+8>=10，因此可以把多乘的 m 利用加 8 进位来替代，同时，只对于当前位数值是 1 的情况，才会发生 n % m + 1的情况

[题44-数字序列中某一位的数字](题44-数字序列中某一位的数字.py)    
找数学规律。比如要求第 1001 位，0-9 只有一位数字，是序列的前 10 位，10-99 有两位数字，是接下来的 180 位，100-999 有三位数字，是接下来的 2700 位，
所以第 811 位是某个三位数中的一位。由于 811 = 3 * 270 + 1，所以第 811 位是从第 100 开始的第 270 个数字即 370 的中间一位，也就是 7

[题45-把数组排成最小的数](题45-把数组排成最小的数.py)    
(1)使用 cmp_to_key 来排序。（2）使用冒泡排序思想。比较每两个数字字符串的拼接 mn 和 nm 的大小，把 m,n 中更小的数放到新列表中，最后输出。
